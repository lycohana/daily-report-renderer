...
日报渲染系统 (Daily Report Renderer) 是一个基于 Node.js + Express 的自动化系统，用于监控指定文件夹中的 Markdown 文件并将其渲染为 HTML 格式的日报页面。
...
| chokidar | 3.6 | 文件监控 |
| node-cache | 5.1 | 内存缓存 |
| sanitize-html | 2.17 | HTML 净化 |

## 自定义标签系统

系统采用模块化标签处理架构，支持三种类型的自定义标签：

### 标签类型

| 类型 | 语法 | 示例 | 处理器目录 |
|------|------|------|------------|
| **行内标签** (inline) | `[标签名：参数]: #` | `[tag:AI]: #` | `handlers/inline/` |
| **标记标签** (marker) | `[标签名]: #` | `[section]: #` | `handlers/marker/` |
| **区块标签** (block) | `<标签名>内容</标签名>` | `<data>...</data>` | `handlers/block/` |
...
| 标签 | 语法 | 作用域 | 描述 |
|------|------|--------|------|
| 标签 | `[tag:名称]: #` | 任意 | 为文章/章节添加标签 |
| 来源链接 | `[from:URL]: #` | 任意 | 设置来源 URL |
| 来源名称 | `[fromstr:名称]: #` | 任意 | 设置来源显示名称 |
| 章节简介 | `[intro:简介]: #` | 章节前 | 设置章节简介 |
| 章节图标 | `[icon:emoji]: #` | 章节前 | 设置章节图标 |
| 总结 | `[sum:摘要]: #` | 章节/文章 | 摘要内容（渲染到末尾） |
| 思考 | `[think:思考]: #` | 章节/文章 | 思考点评（渲染到末尾） |
...
| 头版标记 | `[head]: #` | 标记头版头条开始 |
| 章节标记 | `[section]: #` | 标记章节开始 |
| 文章列表标记 | `[articles]: #` | 标记文章列表开始 |

#### 区块标签

| 标签 | 语法 | 描述 |
|------|------|------|
| 数据块 | `<data>...</data>` | 显示统计数字卡片 |
| 天气块 | `<weather>...</weather>` | 显示天气预报卡片 |
| 总结块 | `<sum>...</sum>` | 原地渲染摘要框 |
| 思考块 | `<think>...</think>` | 原地渲染思考框 |
| 笔记块 | `<notes>...</notes>` | 底部渲染随笔笔记 |
| 引用块 | `> 引用内容` | Markdown 引用块 |
...
{
  customTags: {
    hasHeadMarker: true,           // 是否有 [head]: 标记
    headlineTags: ['AI', 'OpenAI'], // 头版标签
    headFrom: 'https://example.com', // 头版来源
    sectionArticleMeta: [          // 章节/文章元数据
      {
        title: '章节标题',
        intro: '章节简介',
        icon: '🤖',
...
详细标签开发指南见 [docs/tags-dev-guide.md](docs/tags-dev-guide.md)
